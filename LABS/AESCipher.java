...
public class AESCipher {
  ...
  static String[][] w = new String[44][4];
  static String[] o = new String[11];
  static int c;

  public static String sBox(String[] inHex) {
  ...
  public static String Rcon(int round) {
  ...
  public static String[] aesRoundKeys(String KeyIn) {
    int a = 0,b = 2,c = 4; 
    for(int i= 0;i<4;i++){
    for (int j=0;j<4;j++){
    k[i][j]= KeyIn.substring(a,b);
    w[i][j]= KeyIn.substring(a,b);
        a += 2;
	b += 2;
      }
    }
    for (int d = 0; d < 40; d++) {
      if (c % 4 == 0) {
        ...
        for (int i = 0; i < 4; i++) {
          ...
          if (i == 3) {
            temp[i] = sBox(new String[]{Character.toString(w[c - 1][0].charAt(0)), Character.toString(w[c - 1][0].charAt(1))});
          } else {
            temp[i] = sBox(new String[]{Character.toString(w[c - 1][i + 1].charAt(0)), Character.toString(w[c - 1][i + 1].charAt(1))});
          }
        }
        int r = Integer.parseInt(temp[0], 16) ^ Integer.parseInt(Rcon(c / 4), 16);
        temp[0] = Integer.toHexString(r);
        for (int i = 0; i < 4; i++) {
          int f = Integer.parseInt(w[c - 4][i], 16) ^ Integer.parseInt(temp[i], 16);
          if (Integer.toHexString(f).length() < 2) {
            w[c][i] = "0" + Integer.toHexString(f);
          } else {
            w[c][i] = Integer.toHexString(f);
          }
        }
        c++;
      } else {
        for (int i = 0; i < 4; i++) {
          ...
          int holder = Integer.parseInt(w[c - 4][i], 16) ^ Integer.parseInt(w[c - 1][i], 16);
          if (Integer.toHexString(holder).length() < 2) {
            w[c][i] = "0" + Integer.toHexString(holder);
          } else {
            w[c][i] = Integer.toHexString(holder);
          }
        }
        c++;
      }
  ...
  public static String[][] AESNibbleSub(String[][] inStateHex) {
    ...
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 4; j++) {
        ...
        String t = sBox(cha);
        if (t.length() < 2) {
          temp[i][j] = "0" + sBox(cha).toUpperCase();
        } else {
          temp[i][j] = sBox(cha).toUpperCase();
        }
      }
  ...
  public static void AES(String pTextHex, String keyHex) {
    ...
    temp = AESStateXOR(temp, ToMatrix(o[round]));
